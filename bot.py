...
# برای Railway Volume:
# اگر DATA_DIR ست شده باشد از آن استفاده می‌شود
# اگر Railway volume mount path را expose کند از آن استفاده می‌شود
# وگرنه fallback روی /app/data
STORAGE_DIR = Path(
    os.getenv("DATA_DIR")
                ...
                if file_path and os.path.exists(file_path):
                    ...
                    if cached_size_mb > TELEGRAM_FILE_LIMIT_MB:
                        ...
                        if pix_url:
                            ...
                            await query.message.reply_text(f"✅ لینک مستقیم (کش):\n{pix_url}")
                        else:
                            with open(file_path, "rb") as f:
                                await query.message.reply_document(
                                    document=f,
