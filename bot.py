...
ADMIN_ID = int(os.getenv("ADMIN_ID", "0"))  # Telegram ID ادمین
...
def get_user_history(user_id, limit=10):
    ...
    c = conn.cursor()
    c.execute(
        "SELECT link, quality, timestamp FROM downloads WHERE user_id=? ORDER BY timestamp DESC LIMIT ?",
        (user_id, limit),
    )
    rows = c.fetchall()
    conn.close()
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not increment_limit(user_id, max_per_day=10):
...
async def queue_worker(app):
    while True:
        ...
        try:
            ...
            if thumb:
                try:
                    await query.message.reply_photo(thumb, caption="🎬 Thumbnail")
                except Exception:
                    pass

            if size_mb > 48:
                await query.message.reply_text(
                    f"⚠️ File is {int(size_mb)}MB — Telegram cannot send files over 50MB.\n"
                    "🔗 Sending as document (may fail) or use external hosting."
                )
...
def main():
    ...
    app.job_queue.run_once(lambda ctx: asyncio.create_task(queue_worker(app)), 1)
